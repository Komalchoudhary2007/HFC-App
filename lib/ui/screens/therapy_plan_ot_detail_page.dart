...
class TherapyPlanDetailPage extends StatefulWidget {
  final Map<String, dynamic> report;
  const TherapyPlanDetailPage({Key? key, required this.report}) : super(key: key);

  @override
  State<TherapyPlanDetailPage> createState() => _TherapyPlanDetailPageState();
}

class _TherapyPlanDetailPageState extends State<TherapyPlanDetailPage> {
  ...
  Map<String, dynamic> _staticData() => {
    ...
    'gender': 'N/A',
    'dob': 'N/A',
    'diagnosis': _r['description']?.toString() ?? 'Developmental Delay',
    ...
    'goals': [
      {
        ...
        'subDomain': 'Tactile & Vestibular Processing',
        'observation': 'Child shows hypersensitivity to touch and difficulty with balance activities.',
        'shortTerm': 'Tolerate light touch on hands and arms for 2 minutes during structured activities.',
        'longTerm': 'Independently regulate sensory input during daily routines with minimal support.',
        'relevance': 'Sensory regulation is foundational for attention, learning and social participation.',
      },
      {
        'number': 2,
        ...
        'subDomains': [
          {
            'subDomain': 'Pencil Grip & Pre-Writing',
            'observation': 'Child uses fisted grasp and avoids writing tasks.',
            'shortTerm': 'Use tripod grasp for 5 minutes during tracing activities.',
            'longTerm': 'Write name independently with correct pencil grip.',
            'relevance': 'Fine motor skills are essential for academic tasks and self-care.',
          },
          {
            'subDomain': 'Scissor Skills',
            'observation': 'Child cannot open/close scissors independently.',
            'shortTerm': 'Cut along a straight line with minimal assistance.',
            'longTerm': 'Cut simple shapes independently with age-appropriate scissors.',
            'relevance': 'Scissor skills support bilateral coordination and school readiness.',
          },
        ],
      },
    ],
    'approaches': [
      {'title': 'Bilateral & Cross-lateral Movement', 'description': 'Animal walks, ball tapping, and opposite hand-knee tasks to strengthen coordination.'},
      {'title': 'Sensory Regulation Routine', 'description': 'Begin sessions with sensory warm-ups (deep pressure, heavy work) and end with calming input.'},
      {'title': 'Fine Motor & Functional Play', 'description': 'Use bilateral play tools (tongs, lacing, playdough) and progress to self-care tasks.'},
      {'title': 'Parent & Home Involvement', 'description': 'Teach parents daily movement routines and structured sensory breaks for carryover at home.'},
    ],
    'notes': 'Consistent home practice and playful family activities will make therapy progress smoother and more meaningful.\n\nRegular communication between therapist and family ensures alignment on goals and strategies. Progress will be monitored and therapy plan adjusted as needed.',
  };

  String _therapyTitle(String cat) {
    switch (cat) {
      case 'therapy_plan': return 'Occupational Therapy Plan';
      case 'speech_report': return 'Speech Therapy Plan';
      case 'ot_report': return 'Occupational Therapy Plan';
      case 'aba_report': return 'ABA Therapy Plan';
      case 'special_education_report': return 'Special Education Therapy Plan';
      default: return 'Therapy Plan';
    }
  }
  ...
  Widget build(BuildContext context) {
    final category = _r['category']?.toString() ?? '';
    final therapyTitle = _therapyTitle(category);

    return Scaffold(
      backgroundColor: const Color(0xFFF4F6F8),
      appBar: AppBar(
        title: Text(therapyTitle,
  ...
  Widget _buildHeader(String therapyTitle) {
    return ClipRRect(
