...
class AssessmentAbaReportDetailPage extends StatefulWidget {
  final Map<String, dynamic> report;
  const AssessmentAbaReportDetailPage({Key? key, required this.report}) : super(key: key);
  @override
  State<AssessmentAbaReportDetailPage> createState() => _AssessmentAbaReportDetailPageState();
}

class _AbaSection {
  final String title;
  final List<String> lines;
  const _AbaSection({required this.title, required this.lines});
}

class _AbaReportSections {
  ...
  final List<_AbaSection> genericSections;
  const _AbaReportSections({
...
// ─── Parser — mirrors abaReportTemplate.ts logic ─────────────────────────────
// ABA uses colon-terminated headers: "Background Information:", "Recommendations:" etc.
_AbaReportSections _parseAbaReportContent(String raw) {
  final lines = raw
      .replaceAll('\r\n', '\n').replaceAll('\r', '\n')
  ...
  final List<_AbaSection> generic = [];
  ...
  void flush() {
    ...
    final cleanLines = buf.map((l) => l.replaceAll(RegExp(r'^[\-•*]\s*'), '')).where((l) => l.isNotEmpty).toList();
    ...
    } else if (sec.contains('behavioral observation') || (sec.contains('observation') && sec.contains('result'))) {
      behavioral.addAll(buf);
    } else if (sec.contains('clinical impression') || sec.contains('diagnosis') || sec.contains('summary')) {
      clinical.addAll(buf);
...
// ─────────────────────────────────────────────────────────────────────────────
class _AssessmentAbaReportDetailPageState extends State<AssessmentAbaReportDetailPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final content = _r?['reportContent']?.toString() ??
        ...
        _r?['body']?.toString() ?? '';
    final s = _parseAbaReportContent(content);
            ...
            return Stack(clipBehavior: Clip.none, children: [
              Positioned(left: 28, top: 44, right: small ? 120 : 160,
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  const SizedBox(height: 18),
                  const Text('Comprehensive Behavioral Evaluation', maxLines: 2,
  ...
  Widget _buildHeroImage() {
    ...
    return Image.network(
      'https://hireforcare-media.s3.ap-south-1.amazonaws.com/images/report/aba-assessment/aba-therapy-assessment.png',
  ...
  Widget _heroPlaceholder() => Container(
    ...
    child: const Icon(Icons.psychology_rounded, color: Color(0xFF8A63A8), size: 50));

  // ── Client Info — ABA adds license number ────────────────────────────────────
  Widget _buildClientInfo({
    required String childName, required String age, required String gender,
    required String dob, required String asmtDate, required String therapist,
  ...
  // CONTENT SECTIONS
  // ============================================================================
  List<Widget> _buildContentSections(_AbaReportSections s) {
    return [
  ...
  // ── Background Information — ABA uses bullet list ────────────────────────────
