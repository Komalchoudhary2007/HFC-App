...
class TherapyPlanSpeechDetailPage extends StatefulWidget {
  final Map<String, dynamic> report;
  const TherapyPlanSpeechDetailPage({Key? key, required this.report}) : super(key: key);

  @override
  State<TherapyPlanSpeechDetailPage> createState() => _TherapyPlanSpeechDetailPageState();
}

class _TherapyPlanSpeechDetailPageState extends State<TherapyPlanSpeechDetailPage> {
  ...
  Map<String, dynamic> _staticData() => {
    ...
    'gender': 'N/A',
    'dob': 'N/A',
    'diagnosis': _r['description']?.toString() ?? 'Speech and Language Delay',
    ...
    'goals': [
      {
        ...
        'subDomains': [
          {
            'subDomain': 'Vocabulary Development',
            'observation': 'Child uses fewer than 20 words and relies on gestures to communicate.',
            'shortTerm': 'Use 5 new functional words spontaneously during play activities.',
            'longTerm': 'Use 50+ words and 2-word combinations to express needs and wants.',
            'relevance': 'Expressive vocabulary is foundational for communication and social interaction.',
          },
          {
            'subDomain': 'Sentence Structure',
            'observation': 'Child uses single words only; no two-word combinations observed.',
            'shortTerm': 'Combine 2 words (e.g., "more juice", "big ball") in structured activities.',
            'longTerm': 'Use 3-4 word sentences consistently across settings.',
            'relevance': 'Sentence structure supports academic readiness and peer communication.',
          },
        ],
      },
      {
        ...
        'subDomain': 'Speech Sound Production',
        'observation': 'Child omits final consonants and substitutes /t/ for /k/ sounds.',
        'shortTerm': 'Produce /k/ sound correctly in initial position in 80% of trials.',
        'longTerm': 'Produce all target sounds correctly in conversational speech.',
        'relevance': 'Clear articulation improves intelligibility and reduces communication frustration.',
      },
    ],
    'approaches': [
      'Articulation & Phonological Therapy — Target specific speech sounds through structured drills and play-based activities.',
      'Language Stimulation Techniques — Use modeling, expansion, and recasting during functional activities.',
      'Oral-Motor Exercises — Strengthen oral musculature through targeted exercises.',
      'Social Communication Training — Practice turn-taking, eye contact, and conversational skills.',
      'Parent & Caregiver Coaching — Teach families strategies for home practice and language-rich environment.',
    ],
    'reinforcements': [
      'Verbal praise and encouragement after each successful attempt',
      'Preferred toys and activities as motivators during sessions',
      'Sticker charts for tracking progress and celebrating milestones',
    ],
    'notes': 'Consistent home practice and speech-rich interactions will enhance therapy progress and support communication development.\n\nRegular communication between therapist and family ensures alignment on goals and strategies.',
  };
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Speech Therapy Plan',
        ...
        child: Stack(children: [
          ...
          Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('Speech Therapy Plan',
  ...
  // ── SECTION 2: Diagnosis Box — Speech uses "Primary Concern / Diagnosis" ──
  ...
  Widget _buildChildInfoCard() {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        _infoGrid([_infoItem('Gender', gender), _infoItem('DOB', '$dob ($age)'), _infoItem('Therapist', therapist)]),
        const Divider(height: 20, color: Color(0xFFE5E7EB)),
        _infoGrid([_infoItem('Joining Date', joiningDate), _infoItem('Session Frequency', sessionFreq), _infoItem('Mode', mode)]),
      ]),
    );
  }
  ...
  Widget _buildGoalColumns({
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(width: 10),
        // Speech uses "Why This Goal Is Important (Relevance)" label
        Expanded(child: _goalBox('WHY THIS GOAL IS IMPORTANT', relevance,
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Row(children: [
          ...
          SizedBox(width: 10),
          Text('Approaches and Strategies',
  ...
  // ── SECTION 6: Reinforcements (Speech-specific) ───────────────────────────

  Widget _buildReinforcementsSection() {
    final rawReinforcements = _data?['reinforcements'];
    if (rawReinforcements == null || rawReinforcements is! List || rawReinforcements.isEmpty) return const SizedBox.shrink();
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 12),
        ...rawReinforcements.map((r) {
          final text = r.toString();
          return Padding(
            padding: const EdgeInsets.only(bottom: 8),
            child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('• ', style: TextStyle(fontSize: 14, color: Color(0xFFF59E0B), fontWeight: FontWeight.w700)),
