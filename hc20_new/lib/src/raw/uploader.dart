  ...
  Future<void> uploadAllDayRri(List<Map<String, dynamic>> rriEntries, String mac, int packetIndex, int totalPackets) async {
    ...
    for (int batchIndex = 0; batchIndex < batches.length; batchIndex++) {
      ...
      for (final entry in batch) {
        ...
        // Use packet_index from entry if present (for batched uploads), otherwise use parameter
        final entryPacketIndex = entry['packet_index'] as int? ?? packetIndex;
        
        entries.add({
          'device_id': mac,
          'timestamp': timestamp,
          'packet_index': entryPacketIndex,
